...
public class GenericDao<T extends NewsEntity> extends QueryExecutor<T> {
    ...
    public List<T> insert(Connection connection, List<T> entityList) {
        return null;
    }
    ...
    public List<T> select(Connection connection, PredicateClause predicateClause) throws SQLException, IllegalAccessException, IOException, InstantiationException {
        try {
            Map<String, List<Object>> queryStringAndParameters = selectQueryBuilder.getQueryStringAndParameters(predicateClause);
            String sqlQuery = "";
            List<Object> parameters = null;
            for(Map.Entry<String, List<Object>> entry : queryStringAndParameters.entrySet()){
